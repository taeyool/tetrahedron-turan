...
-- Proof template: scripts/exact_certificate/templates/Examples/TetrahedronOrder7Reduce.lean; certificate-dependent imports redirected.
...
noncomputable def oldRootCoeff (H : FlagWithSize TetraFree emptyType 7) : ℚ :=
  ((36 * s1Value (boundedRep H) + 4 * s3Value (boundedRep H) : ℤ) : ℚ)
    / columnDenom

/-- The five-root contribution, with the ordered-root denominator. -/
...
/-- The previously established one-root and three-root contribution. -/
...
theorem certBound_smul_one :
  ...
  have hc : ((certBound : ℚ) : ℝ) = (columnCoeff H : ℝ) + slack7 H := by
    ...
    have h4 : columnDenom = (20160000000000000 : ℚ) := rfl
    rw [h1, h2, h3, h4]
    push_cast
    ring
  rw [hc, add_smul]
...
lemma deckCoeff_eq_sum (H : FlagWithSize TetraFree emptyType 7) :
    deckCoeff H
      = ∑ H₆ : FlagWithSize TetraFree emptyType 6,
          (classNumOf H₆ / 2880000000000000) * subflagDensity H₆ H := by
  ...
  rw [h,
    show (columnDenom : ℚ) = 7 * 2880000000000000 from by
      rw [show columnDenom = (20160000000000000 : ℚ) from rfl]; norm_num,
...
theorem deckElt_eq_certElt6 :
  ...
  have hexp : ∀ H₆ : FlagWithSize TetraFree emptyType 6,
      ((classNumOf H₆ / 2880000000000000 : ℚ) : ℝ) •
          (⟦basisVector (⟨6, H₆⟩ : FinFlag TetraFree emptyType)⟧
            : FlagAlgebra TetraFree emptyType)
        = ∑ H : FlagWithSize TetraFree emptyType 7,
            (((classNumOf H₆ / 2880000000000000) * subflagDensity H₆ H : ℚ) : ℝ) •
  ...
  have hc : ((deckCoeff H : ℚ) : ℝ)
      = ∑ H₆ : FlagWithSize TetraFree emptyType 6,
          (((classNumOf H₆ / 2880000000000000) * subflagDensity H₆ H : ℚ) : ℝ) := by
    rw [deckCoeff_eq_sum]
