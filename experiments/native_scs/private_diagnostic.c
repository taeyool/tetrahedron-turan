...
static scs_int pcg(ScsLinSysWork *pr, const scs_float *s, scs_float *b,
  ...
  scs_int i, n = pr->n;
  SCS(timer) section6_cg_timer;
  scs_float section6_next_report = 30.;
  SCS(tic)(&section6_cg_timer);
  ...
  if (n > 100000) {
    scs_printf("Section6 CG start: n=%lld, tol=%.9e, initial_residual=%.9e, warm=%d\n",
               (long long)n, tol, CG_NORM(r, n), s != SCS_NULL);
  }
...
#endif
    if (n > 100000) {
      scs_float section6_elapsed = SCS(tocq)(&section6_cg_timer) / 1000.;
      if (i == 0 || section6_elapsed >= section6_next_report) {
        scs_printf("Section6 CG progress: iterations=%lld, seconds=%.6f, residual=%.9e, tol=%.9e\n",
                   (long long)i + 1, section6_elapsed, CG_NORM(r, n), tol);
        section6_next_report = section6_elapsed + 30.;
      }
    }
    if (CG_NORM(r, n) < tol) {
      if (n > 100000) {
        scs_printf("Section6 CG complete: iterations=%lld, seconds=%.6f, residual=%.9e, tol=%.9e\n",
                   (long long)i + 1, SCS(tocq)(&section6_cg_timer) / 1000., CG_NORM(r, n), tol);
      }
      return i + 1;
    }
    /* z = M r (M is inverse preconditioner) */
