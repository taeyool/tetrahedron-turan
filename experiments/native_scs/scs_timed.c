...
scs_int scs_solve(ScsWork *w, ScsSolution *sol, ScsInfo *info,
                  scs_int warm_start) {
  scs_int i;
  scs_float section6_max_iteration = 0, section6_iteration_start = 0;
  ...
  if (w->d->n > 100000) {
    scs_printf("Section6 initial work-cache solve starting\n");
  }
  update_work(w, sol);
  if (w->d->n > 100000) {
    scs_printf("Section6 initial work-cache ready after %.6f seconds\n", SCS(tocq)(&solve_timer) / 1000.);
  }
  ...
  for (i = 0; i < w->stgs->max_iters; ++i) {
    section6_iteration_start = SCS(tocq)(&solve_timer) / 1000.;
    ...
    /* Deadline admission only; keep the stock residual/convergence schedule. */
    if (w->stgs->time_limit_secs) {
      scs_float elapsed = SCS(tocq)(&solve_timer) / 1000.;
      section6_max_iteration = MAX(section6_max_iteration, elapsed - section6_iteration_start);
      if (elapsed + 1.15 * section6_max_iteration + 1.0 >= w->stgs->time_limit_secs) {
        w->time_limit_reached = 1;
        populate_residual_struct(w, i);
        break;
      }
