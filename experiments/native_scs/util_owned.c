...
void SCS(deep_copy_data)(ScsData *dest, const ScsData *src) {
  dest->n = src->n;
  dest->m = src->m;
  dest->A = (ScsMatrix *)scs_malloc(sizeof(ScsMatrix));
  if (!dest->A) { fprintf(stderr, "Input ownership allocation failed\n"); exit(70); }
  *(dest->A) = *(src->A);
  src->A->x = SCS_NULL; src->A->i = SCS_NULL; src->A->p = SCS_NULL;
